...
router.get('/:id', (req: AuthRequest, res: Response) => {
  ...
  const quote = db.prepare(`SELECT q.*, c.name as contact_name, comp.name as company_name, u.name as created_by_name
    FROM quotes q LEFT JOIN contacts c ON q.contact_id = c.id
    LEFT JOIN companies comp ON q.company_id = comp.id
    LEFT JOIN users u ON q.created_by = u.id WHERE q.id = ?`).get(req.params.id);
...
router.post('/', (req: AuthRequest, res: Response) => {
  ...
  db.prepare(`INSERT INTO quotes (id, title, value, contact_id, company_id, deal_id, items, notes, created_by) VALUES (?,?,?,?,?,?,?,?,?)`)
    .run(id, title, value || 0, contact_id || null, company_id || null, deal_id || null, JSON.stringify(items || []), notes, req.user!.id);
...
router.put('/:id', (req: AuthRequest, res: Response) => {
  ...
  db.prepare(`UPDATE quotes SET title=COALESCE(?,title), value=COALESCE(?,value), status=COALESCE(?,status), items=COALESCE(?,items), notes=COALESCE(?,notes), updated_at=CURRENT_TIMESTAMP WHERE id=?`)
    .run(title, value, status, items ? JSON.stringify(items) : null, notes, req.params.id);
