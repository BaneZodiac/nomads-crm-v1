...
router.get('/', (req: AuthRequest, res: Response) => {
  ...
  let sql = `SELECT d.*, c.name as contact_name, comp.name as company_name, u.name as owner_name
    ...
    LEFT JOIN users u ON d.owner_id = u.id WHERE 1=1${scope.filter}`;
...
router.get('/stages', (req: AuthRequest, res: Response) => {
  ...
  const deals = db.prepare(`SELECT d.*, c.name as contact_name, comp.name as company_name, u.name as owner_name
    FROM deals d LEFT JOIN contacts c ON d.contact_id = c.id
    LEFT JOIN companies comp ON d.company_id = comp.id
    LEFT JOIN users u ON d.owner_id = u.id WHERE 1=1${scope.filter} ORDER BY d.created_at DESC`).all(...scope.params);
...
router.get('/:id', (req: AuthRequest, res: Response) => {
  ...
  const deal = db.prepare(`SELECT d.*, c.name as contact_name, comp.name as company_name, u.name as owner_name
    FROM deals d LEFT JOIN contacts c ON d.contact_id = c.id
    LEFT JOIN companies comp ON d.company_id = comp.id
    LEFT JOIN users u ON d.owner_id = u.id WHERE d.id = ?${scope.filter}`).get(req.params.id, ...scope.params);
...
router.post('/', (req: AuthRequest, res: Response) => {
  ...
  db.prepare(`INSERT INTO deals (id, title, value, stage, probability, contact_id, company_id, owner_id, notes, expected_close_date, tenant_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
    .run(id, title, value || 0, stage || 'lead', probability || 10, contact_id, company_id, req.user!.id, notes, expected_close_date, req.user!.tenant_id || null);
...
router.put('/:id', (req: AuthRequest, res: Response) => {
  ...
  const scope = tenantScope(req);
  const { title, value, stage, probability, contact_id, company_id, notes, expected_close_date } = req.body;
  db.prepare(`UPDATE deals SET title=COALESCE(?,title), value=COALESCE(?,value), stage=COALESCE(?,stage), probability=COALESCE(?,probability), contact_id=COALESCE(?,contact_id), company_id=COALESCE(?,company_id), notes=COALESCE(?,notes), expected_close_date=COALESCE(?,expected_close_date), updated_at=CURRENT_TIMESTAMP WHERE id=?${scope.filter}`)
    .run(title, value, stage, probability, contact_id, company_id, notes, expected_close_date, req.params.id, ...scope.params);
...
router.delete('/:id', (req: AuthRequest, res: Response) => {
  const db = getDb();
  const scope = tenantScope(req);
  db.prepare(`DELETE FROM deals WHERE id = ?${scope.filter}`).run(req.params.id, ...scope.params);
